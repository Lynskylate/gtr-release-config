...
class TestDeployCtl(unittest.TestCase):
    ...
    def test_validate_stack_rejects_cni_fields_under_host_mode(self) -> None:
        """Stale CNI-only fields (ip_address/network_aliases/extra_hosts) are invalid
        under host mode — validate must reject them rather than silently ignore."""
        base = {
            "apiVersion": "deploy.lynskylate/v1alpha1",
            "kind": "DeploymentStack",
            "service_name": "s",
        ...
        host_stale = dict(base, runtime={"type": "rootless-podman", "network": {"mode": "host"}})
        with self.assertRaises(deployctl.ValidationError):
            deployctl.validate_stack(host_stale)

        # Same fields are fine under bridge mode
        bridge_stale = dict(base, runtime={"type": "rootless-podman", "network": {"name": "x"}})
        deployctl.validate_stack(bridge_stale)  # must not raise
